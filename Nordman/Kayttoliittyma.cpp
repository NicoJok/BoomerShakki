...
void Kayttoliittyma::piirraLauta() {
    // K�y l�pi lauta ylh��lt� alas (rivi 7 -> 0)
    for (int rivi = 7; rivi >= 0; rivi--) {
        ...
        for (int sarake = 0; sarake < 8; sarake++) {
			...
			if ((rivi + sarake) % 2 == 0) {
                ...
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 32); // Vihre�
            }
			...
			if (asema->lauta[rivi][sarake] != nullptr) {
                ...
                std::wcout << L"  "; // Kaksi v�lily�nti�, jotta ruudut pysyv�t tasaisina
            }
...
Siirto Kayttoliittyma::annaVastustajanSiirto() {
    ...
    //Normaali siirto
	//Onko ensimm�inen merkki iso kirjain? Jos on, niin se on nappula, muuten sotilas
...
int Kayttoliittyma::kysyKorotusNappula() {
	std::wcout << L"Valitse korotusnappula (D = Daami, T = Torni, R = Ratsu, L = Lahetti): ";
	...
	//Lis�t��n my�s sy�tteen tarkistus isoille ja pienille kirjaimille, jotta ohjelma ei kaadu
