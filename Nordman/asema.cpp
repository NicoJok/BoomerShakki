...
void Asema::annaLaillisetSiirrot(std::vector<Siirto>& lista) {
	...
	for (const auto& siirto : raakasiirrot) {
		...
		for (int rivi = 0; rivi < 8; rivi++) {
			...
			if (kuningasRivi != -1) {
				break; // Kuningas l�ytyi, ei tarvitse jatkaa
			}
		}
...
bool Asema::onkoRuutuUhattu(int rivi, int sarake, int hyokkaajanVari) {
	//Muodostetaan ensin hy�kk�j�n kaikki siirrot
	std::vector<Siirto> hyokkaykset;

	//K�yd��n lauta l�pi ja etsit��n hy�kk��j�n nappulat
	for (int r = 0; r < 8; r++) {
		for (int s = 0; s < 8; s++) {
			Nappula* nappula = lauta[r][s];

			//Jos nappula l�ytyy ja se on hy�kk��j�n v�rinen
	...
	//Tarkistetaan voiko hy�kk�ys osua kyseiseen ruutuun
