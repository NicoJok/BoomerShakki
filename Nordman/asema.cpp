...
void Asema::paivitaAsema(Siirto* siirto) {
	...
	if (siirrettavaNappula != nullptr &&
		...
		if (etaisyys == 2) {  // Linnoitus
			...
			if (loppuSarake > alkuSarake) {  // Lyhyt linnoitus (oikealle)
				...
				lauta[rivi][7] = nullptr;          // Tyhjenn‰ h
			}
			else {  // Pitk‰ linnoitus (vasemmalle)
				// Torni a1/a8 -> d1/d8
				Nappula* torni = lauta[rivi][0];  // a-sarake (0)
				lauta[rivi][3] = torni;            // d-sarake (3)
				lauta[rivi][0] = nullptr;          // Tyhjenn‰ a
			}
		}
	}
	...
	if (siirrettavaNappula->getKoodi() == VS || siirrettavaNappula->getKoodi() == MS) {
		if (alkuSarake != loppuSarake && syotyNappula == nullptr) {
			//Sotilas liikkui vinottain tyhj‰‰n ruutuun eli en passant toteutui
...
void Asema::annaLaillisetSiirrot(std::vector<Siirto>& lista) {
	...
	//Suodatetaan lailliset siirrot, eli ne jotka eiv‰t j‰t‰ omaa kuningasta uhattavaksi
	for (const auto& siirto : raakasiirrot) {
		//Tehd‰‰n siirto ns v‰liaikaisesti
		Ruutu alkuRuutu = siirto.getAlkuRuutu();
		Ruutu loppuRuutu = siirto.getLoppuRuutu();
		...
		for (int rivi = 0; rivi < 8; rivi++) {
			...
			if (kuningasRivi != -1) {
				break; // Kuningas lˆytyi, ei tarvitse jatkaa
			}
		}

		//Tarkistetaan onko kuningas uhattuna
		int vastustaja = (siirtovuoro == 0) ? 1 : 0; // Valitaan vastustajan v‰ri
		bool onLaillinen = !onkoRuutuUhattu(kuningasRivi, kuningasSarake, vastustaja);

		//Linnoituksen tarkastus
		//Jos siirto on linnoitus, tarkistetaan ett‰ kuningas ei kulje uhattujen ruutujen l‰pi
		...
		//Lis‰t‰‰n vain jos siirto on laillinen
		if(onLaillinen) {
...
bool Asema::onkoRuutuUhattu(int rivi, int sarake, int hyokkaajanVari) {
	//Muodostetaan ensin hyˆkk‰j‰n kaikki siirrot
	std::vector<Siirto> hyokkaykset;

	//K‰yd‰‰n lauta l‰pi ja etsit‰‰n hyˆkk‰‰j‰n nappulat
	for (int r = 0; r < 8; r++) {
		for (int s = 0; s < 8; s++) {
			Nappula* nappula = lauta[r][s];

			//Jos nappula lˆytyy ja se on hyˆkk‰‰j‰n v‰rinen
	...
	//Tarkistetaan voiko hyˆkk‰ys osua kyseiseen ruutuun
